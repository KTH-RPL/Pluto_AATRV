...
class NavigationSystem:
    ...
    def record_data(self, robot_pose, closest_point, closest_idx, goal_distance):
        ...
        robot_heading = self.get_yaw_from_pose(robot_pose)
    ...
    def robot_pose_callback(self, msg):
        ...
        if self.current_path is None and not self.finished:
    ...
    def generate_offset_path(self):
        ...
        rospy.loginfo(f"Generated offset path with {len(path_points)} points {path_points}")
    ...
    def run_control(self):
        try:
            while not rospy.is_shutdown():
                rospy.loginfo(f" reached! {(self.reached)} ")
                if self.current_path is not None:
                    rospy.loginfo(f" Path end! {(self.current_path[-1])} ")
                if self.current_pose is not None:
                    rospy.loginfo(f" Pose! {(self.current_pose)} ")

                if self.current_path is not None and self.current_pose is not None and self.reached is not True:
                    ...
                    rospy.loginfo(f" gOAL {(x_goal,y_goal)} and current {(x_robot,y_robot)}")
